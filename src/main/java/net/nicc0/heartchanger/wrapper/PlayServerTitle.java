...
public class PlayServerTitle extends AbstractPacket {

    public static final PacketType TYPE = PacketType.Play.Server.TITLE;

    public PlayServerTitle() {
        super(new PacketContainer(TYPE), TYPE);
        handle.getModifier().writeDefaults();
    }

    public PlayServerTitle(TitleAction titleAction, WrappedChatComponent chatComponent, int fadeIn, int stay, int fadeOut) {
        super(new PacketContainer(TYPE), TYPE);
        
        handle.getTitleActions().write(0, titleAction);
    ...
    public PlayServerTitle(PacketContainer packet) {
        super(packet, TYPE);
    }
